#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Wave 9: условие внутридневный диапазон / close < thr"""
import pandas as pd
import numpy as np
import os
...
thresholds = np.arange(0.005, 0.051, 0.002)
results = []
for thr in thresholds:
    sig = (df["range_pct"] < thr).astype(int)
...
for i,(sr,tr,thr) in enumerate(top3,1):
    print(f"{i}. Sharpe={sr:.4f}, Trades={tr}, Cond: range_pct < {thr:.3f}")

with open("history.log","a",encoding="utf-8") as f:
    for i,(sr,tr,thr) in enumerate(top3,1):
        f.write(f"{datetime.utcnow().isoformat()} | Wave 9 | rank {i} | Sharpe={sr:.4f} | Trades={tr} | Condition: range_pct < {thr:.3f}\n")