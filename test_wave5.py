#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Wave 5: refine gap threshold around 0.002"""
import pandas as pd
import numpy as np
import os
...
thresholds = np.arange(-0.005, 0.011, 0.0005)  # -0.5% .. +1%

results = []
for thr in thresholds:
    sig = (df["gap"] < thr).astype(int)
...
for i,(sr,tr,thr) in enumerate(top3,1):
    print(f"{i}. Sharpe={sr:.4f}, Trades={tr}, Cond: gap < {thr:.4f}")

with open("history.log","a",encoding="utf-8") as f:
    for i,(sr,tr,thr) in enumerate(top3,1):
        f.write(f"{datetime.utcnow().isoformat()} | Wave 5 | rank {i} | Sharpe={sr:.4f} | Trades={tr} | Condition: gap < {thr:.4f}\n")